...
It breaks when the ICP shifts. The first customers were early adopters with a specific context. The next wave has different language, different objections, different decision criteria. The original positioning was accurate for one cohort and wrong for the next. It breaks when competition moves—your category fills up, a new entrant takes the position you were holding, what made you distinctive last year is table stakes now. It breaks when the product changes: features land, use cases expand, what you can actually do for customers is different from what you said at launch, and positioning lags the product because nobody updates it.
...
It's also invisible. When positioning works, the downstream effects—pipeline, conversion rates, shorter sales cycles—get attributed to sales or marketing. When positioning breaks, the diagnostic instinct is to fix whatever has the numbers: change the sales process, update the ads, retrain the reps. The dependency gets checked last, when it should be checked first.

Before you update messaging, check positioning. Before you retrain sales, check positioning. Before you rebuild the funnel, check positioning. If positioning isn't holding, everything downstream is fighting a dependency that doesn't resolve. You get motion, not progress.
